...
6. Рассмотрим какие атрибуты (claims) добавляет SPI в токен доступа. 
...
После этого Keycloak создаст пользователя с правами администратора для данной организации. В текущей версии Keycloak такое поведение нельзя предотвратить.
...
### Дополнительные атрибуты токена доступа 

SPI добавляет в токен доступа дополнительные атрибуты (claims):
- *organization-attribute* - информация об организациях в которых состоит пользователь 
- *active-organization* - информация об активной компании и ролях пользователя в этой организации 
- *organization-role* - список ролей пользователя для каждой компании в которой он состоит
...
Пользователь может состоять в нескольких организациях. В токене доступа содержатся сведения о всех этих организациях. Информация представлена в виде *KV* коллекции *organization-attribute*. Ключ определяется уникальным идентификатором каждой компании. 
